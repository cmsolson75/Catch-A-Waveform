...
def create_input_signals(params:object, input_signal:np.ndarray, Fs:int):
    ...
    for k in range(n_scales):
        ...
        if downsample == 1:
            ...
            #Fix error
            coarse_sig = torch.Tensor(librosa.resample(y=input_signal.squeeze().numpy(), orig_sr=Fs, target_sr=fs))
...
def get_input_signal(params:object):
    ...
    if params.init_sample_rate < Fs:
        ...
        samples = librosa.resample(y=hr_samples, orig_sr=Fs, target_sr=params.init_sample_rate)
